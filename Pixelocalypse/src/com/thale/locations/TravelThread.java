...
public class TravelThread extends GameAsset implements Runnable
{
	private Game game;
	...
	public TravelThread(Game game, Player player, String location)
	{
		super(game);
		this.game = game;
		this.player = player;
	...
	public void run()
	{
		System.out.print("\n\n" + player.getName() + " has started travelling.");
		...
		System.out.print("\n\n" + player.getName() + " has finished travelling.");
		...
		game.setCurrentLocation(game.getLocationManager().getLocation(location));
		player.getScene().setBgImageStr(game.getRespath() + "scenes/" + player.getLocation() + "Scene.png");
		player.getScene().resetBgImage();
		game.getSceneManager().regenerateScene(player);
		game.getSceneManager().showScene(player);
